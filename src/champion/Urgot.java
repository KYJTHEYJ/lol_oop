...
public class Urgot extends Champion {
    ...
    public void skillQ(Champion target) {
        System.out.println("우르곳의 부식성 폭약!");
        System.out.printf("< %s > 의 대미지를 %d 를 입히고 방어력을 3 감소 시킵니다!\n"
                , target.getName()
                , (int) Math.floor(this.getAttackDamage() * URGOT_INIT_SKILL_Q_COEFFICIENT));
        takeDamage(target, (int) Math.floor(this.getAttackDamage() * URGOT_INIT_SKILL_Q_COEFFICIENT));
    ...
    public void skillW(Champion target) {
        System.out.println("우르곳의 심판의 원!");
        System.out.printf("< %s > 에게 %d차례 대미지를 %d 를 입힙니다!\n"
                , target.getName()
                , URGOT_INIT_SKILL_W_HITS
                , (int) Math.floor(this.getAttackDamage() * URGOT_INIT_SKILL_W_COEFFICIENT));
    ...
    public void skillE(Champion target) {
        System.out.println("우르곳의 경멸!");
        System.out.printf("< %s > 에게 대미지를 %d 를 입히고 HP를 %d 회복합니다!\n"
                , target.getName()
                , (int) Math.floor(this.getAttackDamage() * URGOT_INIT_SKILL_E_COEFFICIENT)
    ...
    public void skillR(Champion target) {
        System.out.println("우르곳의 불사의 공포!");
        System.out.printf("< %s > 에게 대미지를 %d 를 입히고 입힌 후 상대의 체력이 %d 이하라면 상대의 남은 체력 만큼 추가 대미지를 입힙니다\n"
                , target.getName()
                , (int) Math.floor(this.getAttackDamage() * URGOT_INIT_SKILL_R_COEFFICIENT)
